...
template <class L, typename A> void binning(Grid::GridBase *Coarse, std::vector<L,A>  const& in, int T, int n_bin, int block_size, int tau, std::string const e_name){
  ...
  for (int t=0; t<T; t++){
    ...
    std::cout << GridLogMessage << "NVS MFACC " + e_name + " (Binning): " << tau << " " << block_size << " " << n_bin << " " << t << " "
              << ACC << std::endl;
    ...
    if ( t == 0 ) var0 = var;
    std::cout << GridLogMessage << "NVS Variance " + e_name + " (Binning): " << tau << " " << block_size << " " << n_bin << " " << t << " "
...
template <class L, typename A> void binning2(Grid::GridBase *Coarse, std::vector<L,A>  const& in, std::vector<L,A>  const& in2, int T, int n_bin, int block_size, int tau, std::string const e_name){
  ...
  for (int t=0; t<T; t++){
    ...
    std::cout << GridLogMessage << "NVS MFACCC " + e_name + " (Binning2): " << tau << " " << block_size << " " << n_bin << " " << t << " "
              << avg << std::endl;
    ...
    std::cout << GridLogMessage << "NVS Variance " + e_name + " (Binning2): " << tau << " " << block_size << " " << n_bin << " " << t << " "
...
int main(int argc, char **argv) {
  ...
  ComplexField A0(&Grid), A1(&Grid), avg(&Grid), one(&Grid); one=ComplexField::scalar_type(1.0,0.0);
  std::vector<ComplexField> G(total_configs,&Grid), A(total_configs,&Grid);

  std::cout << std::setprecision(15);
  for (int conf_s=CPar.StartConfiguration, i_bin=0, it=0; conf_s+T-1<=CPar.EndConfiguration; conf_s+=T, i_bin++){  

    ///// Compute estimation of gauge avg's
    avg = Zero();
    for (int t=conf_s; t<conf_s+T; t++) {
      fname = file_path +  WFPar.data_name + "_" + std::to_string(tau) + "_" + CPar.conf_prefix + ".";
      readFile(A0, fname + std::to_string(t));

      avg = avg + (1/RealD(T))*A0;
      std::cout << GridLogMessage << "NVS " + WFPar.data_name + " (conf, tau, val):   " << " " << std::to_string(t) << " " << tau << " "
		<<  real(sum(A0))/RealD(Grid.gSites()) << std::endl;
      }
	...
	if (i==0){ //debug
	  Coordinate scoor; for (int mu=0; mu < Nd; mu++) scoor[mu] = 0;
          ...
          std::cout << GridLogMessage << "NVS (at origin) " + WFPar.data_name + " (conf, tau, val):   " << " " << conf_0 << " " << tau << " "
                    <<  a0 << std::endl;
	}
		
	}

	// Compute autocorrelation coeff's at each site
      G[it] = G[it]; //!!!!!!!!!!!!!!!! CHANGED
	
      }// END: loop within a bin for MD time
    }// END: loop over bins
  ...
  // blocking
  std::vector<ComplexField> G_B(arr_size,&Coarse), A_B(arr_size,&Coarse);
  for (int i=0; i<arr_size; i++){
    blockSum(G_B[i],G[i]); blockSum(A_B[i],A[i]);
    G_B[i] = (1/RealD(std::pow(bs,Nd)))*G_B[i]; A_B[i] = (1/RealD(std::pow(bs,Nd)))*A_B[i];
  }

  // sparse sampling
  std::vector<ComplexField> G_s(arr_size,&Coarse), A_s(arr_size,&Coarse);
  for (int i=0; i<arr_size; i++){
...
/* Incorrect binning method
template <class L, typename A> void binning(Grid::GridBase *Coarse, std::vector<L,A>  const& in, int T, int n_bin, int block_size, int tau, std::string const e_name){
  using namespace Grid;

  L avg(Coarse), avg0(Coarse), var(Coarse), var0(Coarse), cov(Coarse), tmp(Coarse);
  for (int t=0; t<T; t++){
    //should compute ratio in each bin first!!!!!!!!!!! 
...
template <class L, typename A> void binning2(Grid::GridBase *Coarse, std::vector<L,A>  const& in, int T, int n_bin, int block_size, int tau, std::string const e_name){
  using namespace Grid;
  
  RealD avg, avg0, var, var0, cov, tmp;
  std::vector<RealD> in_avg(in.size());

  for (int i=0; i<in.size(); i++) in_avg[i] = (1.0/RealD(Coarse->gSites()))*TensorRemove(sum(in[i])).real();
  //should compute ratio in each bin first!!!!!!!!!!!
  // Find avg field over binns
  for (int t=0; t<T; t++){
    avg = 0.0; var = 0.0; cov = 0.0;
    for (int b=0; b<n_bin; b++)
      avg = avg + (1.0/RealD(n_bin))*in_avg[b*T + t];
    if ( t == 0 ) avg0 = avg;
    RealD ACC = avg/avg0;
    std::cout << GridLogMessage << "NVS MFACC " + e_name + " (Binning2): " << tau << " " << block_size << " " << n_bin << " " << t << " "
              << ACC << std::endl;

    // Find variance of Gt/G0 
    for (int b=0; b<n_bin; b++){
      tmp = in_avg[b*T + t] - avg;
      var = var + (1/RealD((n_bin-1)*n_bin))*(tmp*tmp);
      cov = cov + (1/RealD((n_bin-1)*n_bin))*tmp*(in_avg[b*T] - avg0);
    }
    if ( t == 0 ) var0 = var;
    std::cout << GridLogMessage << "NVS Variance " + e_name + " (Binning2): " << tau << " " << block_size << " " << n_bin << " " << t << " "
              << (var/avg/avg + var0/avg0/avg0 - 2*cov/avg/avg0)*ACC*ACC
	      << " " << (var/avg/avg + var0/avg0/avg0)*ACC*ACC << " " << - 2*cov/avg/avg0*ACC*ACC << std::endl;
  }
}
*/
